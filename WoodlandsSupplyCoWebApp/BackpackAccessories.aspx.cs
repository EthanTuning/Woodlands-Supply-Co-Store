    ...
    public partial class BackpackAccessories : System.Web.UI.Page {
        ...
        protected void AddBtn_Click(object sender, EventArgs e) {
            ...
            if (Request["ProductName"].ToString() == "Aventure Medical Kits Mountain Series Hiker Med Kit") {
                ...
                products.Add(new Product(1234, "Aventure Medical Kits Mountain Series Hiker Med Kit", 20, quantity));
            }

            if (Request["ProductName"].ToString() == "Black Biamond Trail Ergo Cork Trekking Poles") {
                int quantity = Int32.Parse(Request["ProductQuantity"].ToString());
                products.Add(new Product(1235, "Black Biamond Trail Ergo Cork Trekking Poles", 80, quantity));
            }

            if (Request["ProductName"].ToString() == "Petzl Tikkina Headlamp") {
                int quantity = Int32.Parse(Request["ProductQuantity"].ToString());
                products.Add(new Product(1236, "Petzl Tikkina Headlamp", 15, quantity));
            }

            if (Request["ProductName"].ToString() == "Playtypus Big Zip LP Reservoir") {
                int quantity = Int32.Parse(Request["ProductQuantity"].ToString());
                products.Add(new Product(1237, "Playtypus Big Zip LP Reservoir", 25, quantity));
            }

            if (Request["ProductName"].ToString() == "REI Co-op Bitty Sack 3 Pack") {
                int quantity = Int32.Parse(Request["ProductQuantity"].ToString());
                products.Add(new Product(1238, "REI Co-op Bitty Sack 3 Pack", 30, quantity));
            }

            if (Request["ProductName"].ToString() == "REI Co-op Traverse 70 Hipbelt") {
                int quantity = Int32.Parse(Request["ProductQuantity"].ToString());
                products.Add(new Product(1239, "REI Co-op Traverse 70 Hipbelt", 30, quantity));
            }

            if (Request["ProductName"].ToString() == "Suunto MC-2G Navigator Compass") {
                int quantity = Int32.Parse(Request["ProductQuantity"].ToString());
                products.Add(new Product(2234, "Suunto MC-2G Navigator Compass", 55, quantity));
            }
